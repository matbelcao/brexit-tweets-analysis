...
def tweetsMapReduce():
    ...
    mapper = Code("""
                function() {
                    for (var idx = 0; idx < this.salient_words.length; idx++) {
                        ...
                        if (this.t_sentiment == "positive"){
                            var value = {
                                positive: this.salient_words[idx].c,
                                neutral: 0,
                                negative: 0
                            };
                        }else if (this.t_sentiment == "neutral"){
                            var value = {
                                positive: 0,
                                neutral: this.salient_words[idx].c,
                                negative: 0
                            };
                        }else if (this.t_sentiment == "negative"){
                            var value = {
                                positive: 0,
                                neutral: 0,
    ...
    reducer = Code("""
                    ...
                    {
                        reducedVal = {positive: 0, neutral: 0, negative: 0};
    ...
    tweetsDB.map_reduce(mapper, reducer, out={"replace": "leave_sentiment_word_counts"},
                       query={"salient_words": {"$exists": True}, "language": "en", "t_stance": "leave"})

    tweetsDB.map_reduce(mapper, reducer, out={"replace": "remain_sentiment_word_counts"},
...
def analysis(correctionCoefficient):
    ...
    fig = make_subplots(rows=2, cols=3, start_cell="bottom-left", subplot_titles=("", "", "", "POSITIVE", "NEUTRAL", "NEGATIVE"), shared_yaxes = True)
    ...
    df = pd.merge(dfLeave,
                  dfRemain,
                  on='word')
    df.columns = ["word", "leave_positive", "leave_neutral", "leave_negative", "leave_sum", "remain_positive", "remain_neutral", "remain_negative", "remain_sum"]
    ...
    df["leave_sum"] = df["leave_positive"] + df["leave_neutral"] + df["leave_negative"]
