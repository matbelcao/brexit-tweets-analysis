...
def tweet_search(api, tweetID):
    """ Function that retrieves the tweet status using tweepy, and returns text and language of it. """
...
def strip_links(text):
    """ Removes the url links from the status text. """
...
def strip_all_entities(text):
    """ Removes the special characters from the status text. """
...
def get_stop_words(lang):
    """ Retrieves the nltk stop-words of a given language. """
...
def get_items_upto_count_bounded(dct, n):
    """ Crops the list of strings keeping only the n most frequent. Keeps the ties if occurs. """
...
def toTuple(counter):
    """ Transforms of a counter into a tuple (word,count) list. """
...
def toCounter(tuple):
    """ Transforms a tuple (word,count) list into a counter. """
